  ...
  return (
    ...
    <div>
      <div className='relative'>
           <img src={image} alt="" className='max-w-sm rouded' />
      
      <span className={`absolute bottom-0 left-0 h-1 bg-blue-500 $
        {loading ? 'w-full transition-all duration-[10s]': 'w-0'}
        `}/>
      </div>
      {/* when loading is false hide this */}
      <p className={!loading? 'hidden' : ''}>Loading....</p>

    </div>


   {/* when image is not loaded this div will be displayed */} 
   {!isImageLoaded && 
   ...
   {/* when image is loaded this div will be displayed */}
   {isImageLoaded && 
